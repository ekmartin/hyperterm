...
class TermGroup_ extends Component {
  ...
  template (css) {
    ...
    const { direction } = termGroup;
    const capitalized = direction.charAt(0) + direction.slice(1).toLowerCase();
    const createSeparator = (index) => <div
      key={index}
      style={{
        flex: '0 0 11px',
      ...
      className={css(`separator${capitalized}`)}
    />;

    const groups = childGroups.reduce((total, child, index) => {
      ...
      // Don't add a separator for the last child:
      if ((childGroups.length - 1) === index) return newTotal;
      return [...newTotal, createSeparator(index)];
    }, []);

    return <div
      className={css('termGroup', direction.toLowerCase())}>
      {groups}
    </div>;
  }
